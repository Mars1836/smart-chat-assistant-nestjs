...
export class FunctionExecutor extends BaseToolExecutor {
  ...
  private readonly functions: Record<
    string,
    (params: any, context: ExecutionContext) => any
  > = {
    ...
    generate_excel: async (params: any) => {
      ...
      const safeFilename =
        (filename || 'export').replace(/[^a-zA-Z0-9-_]/g, '') +
        '_' +
        Date.now() +
        '.xlsx';
      ...
      if (this.fileCleanupProducer) {
        ...
        console.warn(
          'FileCleanupProducer not available, file will NOT be auto-cleaned up via Queue',
        );
      }
    ...
    ocr_extract_text: async (params: any) => {
      ...
      if (!gemini) {
        throw new Error(
          'OCR tool requires Gemini provider. Please configure GOOGLE_AI_STUDIO_API_KEY.',
        );
      }
      ...
      const res = await safeFetchWithSsrfProtection(imageUrl, {
        method: 'GET',
      });
      if (!res.ok) {
        throw new Error(
